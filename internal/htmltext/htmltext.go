...
func ExtractReadableText(html string) string {
	...
	if text := extractWithGoquery(html); strings.TrimSpace(text) != "" {
		return truncate(normalizeWhitespace(text))
	}

	return truncate(normalizeWhitespace(stripHTMLWithRegex(html)))
}
...
func truncate(text string) string {
	...
	return string(runes[:maxExtractLen]) + "…"
}
