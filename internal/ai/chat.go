...
func BuildChatSystemPrompt(aiContext domain.AIContext, mentionContext []string, viewSummary string) string {
	...
	sb.WriteString("- A successful revise_composer_post is the FINAL step: its result is shown in the composer for the user to apply — there is nothing to save, so do not call it again or announce a save. If it reports a character-limit overflow, fix it in ONE more call by SHORTENING that specific account's account_content_override (keep the richer default for higher-limit platforms); never resubmit the same text or copy the default text into the override.\n")
