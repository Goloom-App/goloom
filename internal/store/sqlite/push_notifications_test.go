...
func pushFixture(t *testing.T, s *sqlite.Store, role domain.TeamRole) (domain.User, domain.Team) {
	t.Helper()
	ctx := context.Background()
	user, err := s.UpsertOIDCUser(ctx, "push-"+uuid.NewString(), "push@x", "Push")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSQLite_PushSubscriptionCRUD(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	...
	if _, err := s.UpdatePushSubscriptionEnabled(ctx, "other-user", created.ID, true); !errors.Is(err, domain.ErrPushSubscriptionNotFound) {
		t.Fatalf("update by other user: %v", err)
	}
	if _, err := s.GetPushSubscription(ctx, "other-user", created.ID); !errors.Is(err, domain.ErrPushSubscriptionNotFound) {
		t.Fatalf("get by other user: %v", err)
	}

	if err := s.DeletePushSubscription(ctx, "other-user", created.ID); !errors.Is(err, domain.ErrPushSubscriptionNotFound) {
...
func TestSQLite_PushSubscriptionRetire(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	user, _ := pushFixture(t, s, domain.RoleOwner)
	created, err := s.CreatePushSubscription(ctx, user.ID, newSub(user.ID, "https://push.test/"+uuid.NewString()))
...
func TestSQLite_PushSubscriptionResubscribeReenables(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	user, _ := pushFixture(t, s, domain.RoleOwner)
...
func TestSQLite_ListPushTargetsRoleAndPrefs(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
...
func TestSQLite_TeamNotificationPrefsUpsert(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
...
func TestSQLite_VAPIDKeysRoundTrip(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
...
func TestSQLite_CountOpenReviewItems(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	user, team := pushFixture(t, s, domain.RoleOwner)
...
func TestSQLite_CountUserOpenReviewItems(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	...
	_, stranger := pushFixture(t, s, domain.RoleOwner)
