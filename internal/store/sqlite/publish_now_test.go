...
// ipatch runs PatchScheduledPost, retrying transient sqlite lock contention
// that can appear when two store calls race on the same in-memory DB.
func ipatch(ctx context.Context, s *sqlite.Store, teamID, postID string, patch domain.UpdatePostPatch) error {
	var err error
	for attempt := 0; attempt < 100; attempt++ {
		_, err = s.PatchScheduledPost(ctx, teamID, postID, patch)
		if err == nil {
			return nil
		}
...
// TestSQLite_PatchScheduledPost_PublishNowAtomicVsConcurrentEdit reproduces the
...
func TestSQLite_PatchScheduledPost_PublishNowAtomicVsConcurrentEdit(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	u, _ := s.UpsertOIDCUser(ctx, "pna", "pna@x", "Pna")
	...
	publishNow := func() error {
		return ipatch(ctx, s, team.ID, post.ID, domain.UpdatePostPatch{
	...
	for i := 1; i <= edits; i++ {
		...
		// Publish_now writers start first; their snapshot read still sees the
		// previous content, then linger while the edit below commits.
		...
		if err := ipatch(ctx, s, team.ID, post.ID, domain.UpdatePostPatch{
...
// TestSQLite_PatchScheduledPost_ReviewTransitionMovesDraftAtomically is the
...
func TestSQLite_PatchScheduledPost_ReviewTransitionMovesDraftAtomically(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	u, _ := s.UpsertOIDCUser(ctx, "rev1", "rev1@x", "Rev1")
	team, _ := s.CreateTeam(ctx, u.ID, domain.CreateTeamInput{Name: "rev-" + uuid.NewString(), Description: ""})
	acc, _ := s.CreateAccount(ctx, team.ID, domain.ConnectedAccount{
		Provider: "bluesky", AuthType: domain.AccountAuthTypeAppPassword,
		InstanceURL: "https://b.social", Username: "b", AccessToken: "t",
	})
...
// TestSQLite_PatchScheduledPost_StaleReviewTransitionConflicts reproduces the
...
func TestSQLite_PatchScheduledPost_StaleReviewTransitionConflicts(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	u, _ := s.UpsertOIDCUser(ctx, "rev2", "rev2@x", "Rev2")
	team, _ := s.CreateTeam(ctx, u.ID, domain.CreateTeamInput{Name: "rev-" + uuid.NewString(), Description: ""})
	acc, _ := s.CreateAccount(ctx, team.ID, domain.ConnectedAccount{
		Provider: "bluesky", AuthType: domain.AccountAuthTypeAppPassword,
		InstanceURL: "https://b.social", Username: "b", AccessToken: "t",
