...
func TestSQLite_ListTeamInvitations(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	owner, err := s.UpsertOIDCUser(ctx, "inv-owner", "inv-owner@x", "Owner")
	if err != nil {
		t.Fatal(err)
	}
	...
	inv1, _, err := s.CreateTeamInvitation(ctx, team.ID, owner.ID, domain.CreateTeamInvitationInput{Email: "one@x", Role: domain.RoleEditor})
	if err != nil {
		t.Fatal(err)
	}
	inv2, _, err := s.CreateTeamInvitation(ctx, team.ID, owner.ID, domain.CreateTeamInvitationInput{Email: "two@x", Role: domain.RoleViewer})
	if err != nil {
		t.Fatal(err)
	}
	...
	if !ok || got1.Email != "one@x" || got1.Role != domain.RoleEditor || got1.TeamID != team.ID || got1.CreatedByUserID != owner.ID {
		t.Fatalf("invitation 1 mismatch: %+v", got1)
	}
	got2, ok := byID[inv2.ID]
	if !ok || got2.Email != "two@x" || got2.Role != domain.RoleViewer {
...
func TestSQLite_DeleteTeamInvitation(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	owner, err := s.UpsertOIDCUser(ctx, "del-owner", "del-owner@x", "Owner")
	if err != nil {
		t.Fatal(err)
	}
	...
	inv, _, err := s.CreateTeamInvitation(ctx, team.ID, owner.ID, domain.CreateTeamInvitationInput{Email: "gone@x", Role: domain.RoleViewer})
	if err != nil {
		t.Fatal(err)
	}

	// Deleting through the wrong team must not remove the invitation.
