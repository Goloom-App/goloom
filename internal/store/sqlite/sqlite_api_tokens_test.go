...
func TestSQLite_ListUserAPITokens_purgesExpiredWebSessions(t *testing.T) {
	...
	if _, _, err := s.CreateUserAPIToken(ctx, u.ID, domain.WebSessionAPITokenName, &past); err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := s.CreateUserAPIToken(ctx, u.ID, "expired-ci", &past); err != nil {
		t.Fatal(err)
	}
	if _, _, err := s.CreateUserAPIToken(ctx, u.ID, "active-ci", &future); err != nil {
		t.Fatal(err)
	}
