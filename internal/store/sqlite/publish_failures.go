...
func (s *Store) AdminListPublishFailures(ctx context.Context) ([]domain.PublishFailure, error) {
	...
	for rows.Next() {
		...
		var scheduledAt, updatedAt string
		if err := rows.Scan(&f.PostID, &f.TeamID, &f.TeamName, &f.Title, &scheduledAt,
			&f.AttemptCount, &f.LastError, &updatedAt); err != nil {
			return nil, err
		}
		f.ScheduledAt, _ = parseTime(scheduledAt)
...
func (s *Store) AdminAcknowledgeFailedPost(ctx context.Context, postID string) (bool, error) {
	...
	res, err := s.db.ExecContext(ctx, `
		update scheduled_posts set acknowledged_at = ?, updated_at = ?
		where id = ? and status = 'failed' and acknowledged_at is null`,
		now, now, postID)
	if err != nil {
		return false, err
	}
...
func (s *Store) AdminRetryFailedPost(ctx context.Context, postID string) (bool, error) {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		where post_id = ? and status <> 'posted'`, postID); err != nil {
		return false, err
	}
